  ...
  async openSlot(appointmentDate: string): Promise<void> {
    ...
    try {
      ...
      if (axiosError.response) {
        ...
        if (import.meta.env.DEV) {
          console.error('Slot creation failed - API Response:', {
        ...
        if (status === 409) {
          errorMessage = `Time slot conflict: ${errorMessage || 'This time slot is already booked'}`;
        } else if (status === 400) {
          errorMessage = `Invalid request: ${errorMessage || 'Please check the date and time format'}`;
        } else if (status === 403) {
          errorMessage = `Access denied: ${errorMessage || 'You do not have permission to create slots'}`;
        } else if (status === 500) {
          errorMessage = `Server error: ${errorMessage || 'Please try again later'}`;
        }
      } else if (axiosError.request) {
        if (import.meta.env.DEV) {
          console.error('Slot creation failed - No response received:', {
            datetime: appointmentDate,
        ...
        if (import.meta.env.DEV) {
          console.error('Slot creation failed - Request setup error:', {
            datetime: appointmentDate,
